...
first 2 numbers are 1 by definition, and each subsequent number is the sum of the 
...
at an incredibly rapid rate. For example, the 100th Fibonacci number 
...
def find_fibonacci_index_by_length(input)
  fib_array = [1, 1]
  until fib_array[-1].to_s.length == input
